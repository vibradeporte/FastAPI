...
def modulos_curso(id: str = Query(max_length=max_lenght_username), curso: str = Query(max_length=max_lenght_courseshortname)):
    with engine.connect() as connection:
        consulta_sql = text(f"""
            SELECT  subconsulta.quiz_id as quiz_id, subconsulta.quiz_name as quiz_name,
                    subconsulta.attempt_count as quiz_attempt_count, subconsulta.Progress as quiz_status
            FROM    (SELECT q.id AS quiz_id, q.name AS quiz_name, COUNT(qa.id) AS attempt_count,
                    ...
                    LEFT JOIN   mdl_user u ON (qa.userid = u.id) AND (u.username = :id)
                    JOIN    mdl_course_modules_completion cmc ON cmc.userid = u.id
                    JOIN    mdl_course_modules cm ON (cmc.coursemoduleid = cm.id) AND (cm.course = c.id) AND (q.id = cm.instance)
                    JOIN    mdl_modules m ON cm.module = m.id
                    GROUP BY q.name) as subconsulta
            WHERE   (subconsulta.attempt_count>=4) AND (subconsulta.Progress='Completed with Fail');
